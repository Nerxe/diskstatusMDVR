...
try {
    ...
    if (fs.existsSync(fleetFile)) {
        const fleetLines = fs.readFileSync(fleetFile, 'utf-8').split('\n');
        for (let i = 1; i < fleetLines.length; i++) {
            const line = fleetLines[i].trim();
            if (!line) continue;
            const parts = line.split(';');
            if (parts.length >= 7) {
                const deviceNameFull = parts[0].trim();
                const fleetPart = parts[6].trim();
    ...
    for (const file of diskAlarmFiles) {
        ...
        fs.writeFileSync(jsonPath, JSON.stringify(processed));
